...
app.get("/ajax/data", (req, res) => {
    ...
    let page = req.query.page || 1;
    const offset = (page - 1) * limit;
    select_query = `select * from Student.Student_Express s INNER JOIN University_Mst u ON s.University = u.id order by sid LIMIT ${offset}, ${limit}  `;
    total_pages = Math.ceil(total_records / limit);

    connection.query(select_query, (err, ans) => {
        if (err) return console.log(err.message);
        res.render("ajax_call", { data: ans, page_no: page, total_pages: total_pages });
    })
});
...
app.listen(7003, (req, res) => console.log("App is running"));
