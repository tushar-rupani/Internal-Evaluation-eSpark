...
const paragraphs = [
    ...
    "Since early times the Indian subcontinent appears to have provided an attractive habitat for human occupation. Toward the south it is effectively sheltered by wide expanses of ocean, which tended to isolate it culturally in ancient times, while to the north it is protected by the massive ranges of the Himalayas, which also sheltered it from the Arctic winds and the air currents of Central Asia. Only in the northwest and northeast is there easier access by land",
    ...
    "East of the latter is the Great Indian, or Thar, Desert, which is in turn bounded on the east by a hill system known as the Aravali Range, the northernmost extent of the Deccan plateau region. Beyond them is the hilly region of Rajasthan and the Malwa Plateau. To the south is the Kathiawar Peninsula, forming both geographically and culturally an extension of Rajasthan. All of these regions have a relatively denser population than the preceding group."
];
const timeTag = document.querySelector(".time span b");
const mistakeTag = document.querySelector(".mistake span")
const cpmTag = document.querySelector(".cpm span")
const wpmTag = document.querySelector(".wpm span")
function loadParagrah() {
    const randomIndex = Math.floor(Math.random() * paragraphs.length);
    typingText.innerHTML = '';
    paragraphs[randomIndex].split("").forEach(word => {
...
let timer, maxTime = 60, timeLeft = maxTime, isTyping = mistakes = charIndex = 0;

function initTyping() {
    let characters = typingText.querySelectorAll("span");
    let typedText = inpField.value.split("")[charIndex];
