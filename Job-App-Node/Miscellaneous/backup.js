...
app.get("/", (req, resp) => {
    connection.query(`select * from Options_Master where select_id = 1`, (err, ans) => {
    ...
    connection.query(`select * from Options_Master where select_id = 5`, (err, ans) => {
        if (err) return console.log(err.message);
        location_data = ans
        resp.render("index", { state_data: state_data, course_data, lang_data, tech_data, location_data })
    })
})
...
app.post("/data", (req, resp) => {
    ...
    var count_sql = `select count(*) as data from personal_info`;
    connection.query(count_sql, (err, res) => {
        id_data = res[0].data + 1;
        var personal_sql = `INSERT INTO personal_info (id, first_name, last_name, contact_no, address, email, gender, dob, created_at) VALUES(${id_data},'${req.body.fname}', '${req.body.lname}', '${req.body.ph}', '${final_addr}', '${req.body.email}', '${req.body.gender}', '${req.body.date}', NOW())`;
